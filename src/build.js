...
function Build(deps = Depends()) {
  this.deps = deps;
  let buildInstance = this;
  var chcpContext;
  ...
  this.execute = function(context) {
    ...
    return new Promise((resolve, reject) => {
      var config = this.prepareConfig(context),
        ignore = context.ignoredFiles;
      if (config == null) {
        reject({ "error": "no configuration file found. Try adding --generate-config-if-missing on command line if you want to generate an in-place configuration", "context": context});
        return;
      }
      deps.recursive(chcpContext.sourceDirectory, ignore, function(err, files) {
        var hashQueue = buildInstance.prepareFilesHashQueue(files);

        deps.async.parallelLimit(hashQueue, 10, function(err, result) {
          result.sort((a, b) => {
          ...
          deps.fs.writeFile(manifestFile, json, function(err) {
            if (err) {
            ...
            deps.fs.writeFile(chcpContext.projectsConfigFilePath, json, function(err) {
              if (err) {
                return console.log(err);
              }
  ...
  this.prepareFilesHashQueue = function(files) {
    var queue = [];
    for (var i in files) {
      var file = files[i];
  ...
  this.prepareConfig = function(context) {
    var config = {};

    try {
      config = deps.fs.readFileSync(context.defaultConfig, 'utf8');
      config = JSON.parse(config);
      config.release = process.env.VERSION || calculateTimestamp();
    } catch (e) {
      if (!context.generateConfigIfMissing) {
        return null;
      }
      config = {
        autogenerated: true,
  ...
  this.hashFile = function(filename, callback) {
    var hash = deps.crypto.createHash('md5'),
      stream = deps.fs.createReadStream(filename);
    ...
    stream.on('end', function() {
      var result = hash.digest('hex'),
        file = deps.path.relative(chcpContext.sourceDirectory, filename).replace(new RegExp("\\\\", "g"), "/");
...
module.exports = function(objArgs) {
  return new Build(Depends(objArgs));
};
